...
def render_industry_admin():
    ...
    return f"<div class='industry-admin-shell'><div class='admin-portal-bar'><div class='admin-portal-title'>Government workspace</div><nav class='admin-portal-links'><a href='/admin'>Moderation</a><a class='active' href='/industry-admin'>Industry partners</a><a href='/universities'>Universities</a><a href='/government-dashboard'>Analytics</a></nav></div><div class='admin-intro'><p class='eyebrow'>Industry verification</p><h1>Review industry partner requests.</h1><p>Approve organizations before they access approved civic challenges and submit support commitments.</p></div><div class='admin-stats'><div><strong>{pending_count}</strong><span>Pending requests</span></div><div><strong>{len(partners)}</strong><span>Total partners</span></div><div><strong>{len(offers)}</strong><span>Support offers</span></div></div><details class='admin-collapsible'><summary><span><b>Partner registrations</b><small>Review and approve industry partner requests.</small></span><i>⌄</i></summary><section class='admin-section'><div class='section-heading'><div><p class='eyebrow'>Onboarding queue</p><h2>Partner registrations</h2></div><span class='queue-label'>{pending_count} awaiting review</span></div><div class='partner-grid'>{partner_markup}</div></section></details><details class='admin-collapsible'><summary><span><b>Support commitments</b><small>Track offers and update their delivery status.</small></span><i>⌄</i></summary><section class='admin-section'><p class='eyebrow'>Collaboration monitoring</p><h2>Support commitments</h2><div class='commitment-list'>{offer_markup}</div></section></details><details class='admin-collapsible'><summary><span><b>Manual onboarding</b><small>Add an industry partner directly as an administrator.</small></span><i>⌄</i></summary><section class='admin-section'><p class='eyebrow'>Manual onboarding</p><h2>Register a partner</h2><form class='industry-create admin-form'><input name='name' placeholder='Organization name' required><select name='partner_type'><option>Industry</option><option>Startup</option><option>MSME</option><option>CSR Organization</option><option>Research Laboratory</option></select><input name='district' placeholder='District' required><input name='domains' placeholder='Domains' required><input name='contact_email' type='email' placeholder='Contact email' required><button type='submit'>Register partner</button></form></section></details></div>"
def render_university_issues():
    universities = load_universities()
    assignments = load_assignments()
    directory = "<details class='admin-collapsible'><summary><span><b>University registration</b><small>Add a new institution for admin approval.</small></span><i>⌄</i></summary><form class='university-create'><input name='name' placeholder='University name' required><input name='district' placeholder='District' required><input name='domains' placeholder='Domains' required><input name='expertise' placeholder='Expertise keywords' required><input name='departments' placeholder='Departments'><input name='laboratories' placeholder='Laboratories'><input name='incubation_facilities' placeholder='Incubation facilities'><input name='contact_email' placeholder='Contact email' required><button class='save-button' type='submit'>Register university</button></form></details><details class='admin-collapsible'><summary><span><b>University profiles</b><small>Edit institution details and access status.</small></span><i>⌄</i></summary>" + "".join(
        f"<form class='university-profile'><input type='hidden' name='university_id' value='{university['id']}'><input name='name' value='{html.escape(university['name'])}' required><input name='district' value='{html.escape(university['district'])}' required><input name='domains' value='{html.escape(university['domains'])}' required><input name='expertise' value='{html.escape(university.get('expertise') or '')}' placeholder='Expertise keywords'><input name='departments' value='{html.escape(university.get('departments') or '')}' placeholder='Departments'><input name='laboratories' value='{html.escape(university.get('laboratories') or '')}' placeholder='Laboratories'><input name='incubation_facilities' value='{html.escape(university.get('incubation_facilities') or '')}' placeholder='Incubation facilities'><input name='contact_email' value='{html.escape(university.get('contact_email') or '')}'><button class='save-button' type='submit'>Save profile</button></form><form class='approval' data-kind='university' data-id='{university['id']}'><span>Approval: {html.escape(university.get('approval_status', 'Active'))}</span><select name='status'><option>Active</option><option>Rejected</option><option>Pending</option></select><button class='save-button'>Save approval</button></form>"
        for university in universities
    ) + "</details>"
    approved = [issue for issue in ISSUES if issue.get("moderation_status", "Pending") == "Approved"]
    if not approved:
        return directory + "<details class='admin-collapsible'><summary><span><b>Approved challenge assignments</b><small>Assign approved civic challenges to universities.</small></span><i>⌄</i></summary><p class='admin-empty'>No approved issues are ready for university assignment.</p></details>"
