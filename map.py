...
LOGIN_PAGE = r"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Sign in · Civic Map</title><style>:root{--ink:#172b28;--muted:#667773;--paper:#f5f1e8;--card:#fffdf8;--accent:#e65f38;--line:#dedbd1}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;color:var(--ink);background:radial-gradient(circle at 10% 15%,rgba(230,95,56,.12),transparent 28%),radial-gradient(circle at 90% 85%,rgba(49,124,145,.1),transparent 30%),var(--paper);font-family:Georgia,serif}.login{width:min(440px,100%);padding:42px;background:rgba(255,253,248,.97);border:1px solid var(--line);border-radius:18px;box-shadow:0 24px 60px rgba(23,43,40,.12),0 4px 12px rgba(23,43,40,.05);position:relative;overflow:hidden}.login:before{content:"";position:absolute;top:0;left:0;right:0;height:5px;background:linear-gradient(90deg,var(--accent),#c48622,#317c91)}.brand{display:flex;align-items:center;gap:12px;margin-bottom:28px}.brand-mark{width:42px;height:42px;display:grid;place-items:center;border-radius:12px;background:var(--ink);color:white;font:700 17px Arial,sans-serif}.brand-name{font:700 14px Arial,sans-serif;letter-spacing:.4px}.eyebrow{margin:0 0 8px;color:var(--accent);font:700 11px Arial,sans-serif;letter-spacing:2px;text-transform:uppercase}h1{margin:0;font-size:44px;line-height:1.05;letter-spacing:-1.5px;font-weight:500}.intro{margin:14px 0 28px;color:var(--muted);font:14px Arial,sans-serif;line-height:1.65}.error{margin:0 0 18px;padding:12px 14px;border:1px solid #efb9aa;border-radius:9px;background:#fff0eb;color:#8b2e1c;font:13px Arial,sans-serif}label{display:block;margin-top:18px;color:var(--ink);font:600 12px Arial,sans-serif}input{display:block;width:100%;margin-top:8px;padding:13px 14px;border:1px solid var(--line);border-radius:9px;outline:none;background:white;color:var(--ink);font:14px Arial,sans-serif;transition:border-color .2s,box-shadow .2s,background .2s}input::placeholder{color:#9a9f9c}input:hover{border-color:#c5c9c4}input:focus{border-color:var(--accent);background:#fffefa;box-shadow:0 0 0 4px rgba(230,95,56,.11)}button{width:100%;margin-top:26px;padding:14px 16px;border:0;border-radius:9px;background:var(--ink);color:white;font:700 13px Arial,sans-serif;letter-spacing:.3px;cursor:pointer;transition:transform .15s,background .2s,box-shadow .2s}button:hover{background:#24413c;transform:translateY(-1px);box-shadow:0 8px 18px rgba(23,43,40,.18)}button:active{transform:translateY(0)}.demo{margin-top:26px;padding:14px 15px;border:1px solid var(--line);border-radius:9px;background:#f1ece2;color:var(--muted);font:12px/1.5 Arial,sans-serif}.demo b{color:var(--ink)}.footer{margin-top:22px;text-align:center;color:#8a918d;font:11px Arial,sans-serif}@media(max-width:520px){body{padding:16px}.login{padding:30px 24px;border-radius:14px}h1{font-size:36px}.brand{margin-bottom:22px}}</style></head><body><main class="login"><div class="brand"><div class="brand-mark">C</div><div class="brand-name">Civic Map</div></div><p class="eyebrow">Civic platform</p><h1>Welcome back.</h1><p class="intro">Sign in to report local issues, discover community challenges, and help turn problems into action.</p>__ERROR__<form method="post" action="/login"><label>Email<input name="email" type="email" autocomplete="username" placeholder="you@example.com" required></label><label>Password<input name="password" type="password" autocomplete="current-password" placeholder="Enter your password" required></label><button type="submit">Sign in to Civic Map</button></form><div class="demo"><b>Local demo account</b><br>citizen@example.com &nbsp;·&nbsp; map123</div><div class="footer">Community issues · Local voices · Collective action</div></main></body></html>"""
MAP_PAGE = (PAGE
...
class MapHandler(BaseHTTPRequestHandler):
	...
	def do_GET(self) -> None:
		...
		if path == "/login":
			page = LOGIN_PAGE.replace("__ERROR__", "").replace("</form>", '</form><a class="link" href="/register">New here? Create an account</a>')
			self.send_html(page)
			return
		if path == "/register":
	...
	def do_POST(self) -> None:
		...
		if not authenticate(email, password):
			page = LOGIN_PAGE.replace("__ERROR__", '<p class="error">Email or password is incorrect.</p>').replace("</form>", '</form><a class="link" href="/register">New here? Create an account</a>')
