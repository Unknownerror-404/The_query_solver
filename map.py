...
def render_industry_dashboard(user):
    ...
    profile_html = (
        ...
        f"<div class='industry-meta'><span class='industry-chip'><strong>{html.escape(partner['partner_type'])}</strong></span><span class='industry-chip'>District · <strong>{html.escape(partner['district'])}</strong></span><span class='industry-chip'>Domains · <strong>{html.escape(partner['domains'])}</strong></span><span class='industry-chip'>Contact · <strong>{html.escape(partner['contact_email'])}</strong></span></div>"
        f"</div><div class='industry-stats'>"
        f"<div class='industry-stat'><strong>{total_offers}</strong><span>Support offers</span></div>"
        f"<div class='industry-stat'><strong>₹ {total_funding:,}</strong><span>Funding pledged</span></div>"
    ...
    for offer in offers:
        ...
        if issue_team:
            ...
            for m in milestones:
                ...
                m_items.append(f"<li><strong>{html.escape(m['title'])}</strong> · {html.escape(m['status'])} · {html.escape(str(m['due_date'] or 'No deadline'))}{t_res}</li>")
            m_markup = "".join(m_items)
            team_markup = f"<div class='industry-team'><p><strong>University team:</strong> {html.escape(issue_team['name'])} · Mentor: <strong>{html.escape(issue_team['faculty_mentor'])}</strong> · Stage: <strong>{html.escape(issue_team['status'])}</strong></p><details><summary>View active milestones ({len(milestones)})</summary><ul>{m_markup or '<li>No milestones defined</li>'}</ul></details></div>"
        ...
        funding_badge = f"<span class='industry-fact'>Funding · ₹ {int(offer.get('funding_amount') or 0):,}</span>" if offer.get('funding_amount') else ""
        resources_line = f"<span class='industry-fact'>Resources · {html.escape(str(offer.get('resources') or ''))}</span>" if offer.get('resources') else ""
        timeline_line = f"<span class='industry-fact'>Timeline · {html.escape(str(offer.get('timeline') or ''))}</span>" if offer.get('timeline') else ""
    ...
    for issue in approved_issues:
        ...
        if assignment:
            ...
            team_info = f"<br>Faculty mentor / team: <strong>{html.escape(issue_team['faculty_mentor'])}</strong> · Stage: <strong>{html.escape(issue_team['status'])}</strong>" if issue_team else "<br><em>Team forming in progress</em>"
            academic_info = f"<div class='industry-academic'><strong>Assigned institution:</strong> {uni_name} · Request status: <strong>{html.escape(assignment['status'])}</strong>{team_info}</div>"
        reports_markup = ""
        if issue_reports:
...
class MapHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/industry-dashboard":
            ...
            self.send_html(template.replace("__CONTENT__", render_industry_dashboard(user)).replace("__USER__", html.escape(user)))
            return
        if path == "/government-dashboard":
            ...
            self.send_html(template.replace("__CONTENT__", render_government_dashboard()).replace("__USER__", html.escape(user)))
            return
