...
LOGIN_PAGE = r"""<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Sign in · Civic Map</title><style>
:root{--ink:#172b28;--muted:#667773;--paper:#f5f1e8;--accent:#e65f38;--line:#d9d7cd}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;color:var(--ink);background:radial-gradient(circle at 15% 20%,#e8d7c1 0 12%,transparent 30%),var(--paper);font-family:Georgia,serif}.login{width:min(420px,calc(100% - 36px));padding:38px;background:#fffdf8;border:1px solid var(--line);box-shadow:12px 12px 0 #d9d7cd}.eyebrow{margin:0 0 8px;color:var(--accent);font:700 11px Arial,sans-serif;letter-spacing:1.8px;text-transform:uppercase}h1{margin:0 0 10px;font-size:42px;font-weight:500}p{color:var(--muted);line-height:1.5}label{display:block;margin-top:18px;font:13px Arial,sans-serif}input{display:block;width:100%;margin-top:6px;padding:12px;border:1px solid var(--line);background:white;font:14px Arial,sans-serif}button{width:100%;margin-top:24px;padding:12px;border:0;background:var(--accent);color:white;font-weight:bold;cursor:pointer}.error{padding:10px;background:#f8ddd4;color:#8b2e1c;font:13px Arial,sans-serif}.demo{margin-top:22px;padding-top:16px;border-top:1px solid var(--line);font:12px Arial,sans-serif}</style></head><body><main class="login"><p class="eyebrow">Civic map</p><h1>Welcome back.</h1><p>Sign in to enter the public record and add a voice to your city.</p>__ERROR__<form method="post" action="/login"><label>Email<input name="email" type="email" autocomplete="username" required></label><label>Password<input name="password" type="password" autocomplete="current-password" required></label><button type="submit">Enter the map</button></form><div class="demo">Local demo: <b>citizen@example.com</b> / <b>map123</b></div></main></body></html>"""
